...
SPEC = importlib.util.spec_from_file_location('publish_v141', ROOT/'.github/scripts/publish_v141.py')
...
class ReleaseChecks(unittest.TestCase):
    def test_intel_mpi_both_compilers_are_required(self):
        ...
        self.assertEqual(sum(map(len, publisher.CHECKS.values())), 11)
...
class PublicationMetadata(unittest.TestCase):
    def setUp(self):
        ...
        Path('VERSION').write_text('1.4.1\n')
        Path('CHANGELOG.md').write_text('## [1.4.1] - 2026-09-26\n')
        Path('CITATION.cff').write_text('version: 1.4.1\nurl: https://example.test/releases/tag/v1.4.1\n')
        for name in ('vaspberry.f','vaspberry_gfortran_serial.f'):
            Path(name).write_text('! PROGRAM VASPBERRY Version 1.4.1\nVASPBERRY (Ver 1.4.1)\n')
        for name in ('docs/VALIDATION_1.4.1.md','docs/releases/v1.4.1.md','examples/features/procar-character/README.md'):
    ...
    def test_version_runtime_changelog_citation_or_missing_example_rejected(self):
        for path,replacement in [('VERSION','1.3.0'),('CHANGELOG.md','## [Unreleased]'),
                                 ('CITATION.cff','version: 1.4.1rc1'),
                                 ('vaspberry.f','! Version 1.4.1\nVASPBERRY (Ver 1.3.0)'),
    ...
    def test_release_links_pin_tag_and_reject_escaping_repo(self):
        notes=Path('docs/releases/v1.4.1.md')
        notes.write_text('[example](../../examples/features/procar-character/README.md#commands)\n')
        result=publisher.release_body(notes,TARGET,[])
        self.assertIn('/blob/v1.4.1/examples/features/procar-character/README.md#commands',result)
