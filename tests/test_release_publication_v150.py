...
SPEC = importlib.util.spec_from_file_location('publish_v150', ROOT/'.github/scripts/publish_v150.py')
...
class PublicationMetadata(unittest.TestCase):
    def setUp(self):
        ...
        Path('CHANGELOG.md').write_text('## [1.5.0] - 2026-09-26\n')
        Path('CITATION.cff').write_text('version: 1.5.0\nurl: https://example.test/releases/tag/v1.5.0\n')
        for name in ('vaspberry.f','vaspberry_gfortran_serial.f'):
            Path(name).write_text('! PROGRAM VASPBERRY Version 1.5.0\nVASPBERRY (Ver 1.5.0)\n')
        for name in ('docs/VALIDATION_1.5.0.md','docs/releases/v1.5.0.md',
                     'tools/vaspberry_post.py','tools/postprocess_config.py',
    ...
    def test_release_and_previous_source_are_pinned(self):
        self.assertEqual(publisher.VERSION, '1.5.0')
        self.assertEqual(publisher.TAG, 'v1.5.0')
        self.assertEqual(publisher.PREVIOUS_TAG, 'v1.4.2')
        self.assertEqual(publisher.PREVIOUS_COMMIT, '0a3e98680eb4688aad5abd2a2a6ac8ab0bd74a7e')

    def test_frontend_guide_and_runnable_examples_must_be_packaged(self):
        for name in ('tools/vaspberry_post.py', 'tools/postprocess_config.py',
                     'docs/POSTPROCESSING.md',
                     'examples/features/simple-postprocess/README.md',
                     'examples/features/simple-postprocess/bi.ini',
                     'examples/features/simple-postprocess/bi-rescan.ini'):
    ...
    def test_version_runtime_changelog_citation_or_missing_example_rejected(self):
        for path,replacement in [('VERSION','1.3.0'),('CHANGELOG.md','## [Unreleased]'),
                                 ('CITATION.cff','version: 1.5.0rc1'),
                                 ('vaspberry.f','! Version 1.5.0\nVASPBERRY (Ver 1.3.0)'),
    ...
    def test_release_links_pin_tag_and_reject_escaping_repo(self):
        notes=Path('docs/releases/v1.5.0.md')
        notes.write_text('[example](../../examples/features/procar-character/README.md#commands)\n')
        result=publisher.release_body(notes,TARGET,[])
        self.assertIn('/blob/v1.5.0/examples/features/procar-character/README.md#commands',result)
