...
!                 n-th (to m-th) eigenstate using Kubo formula
...
! Validate every spin/k before any bundle output is created.
     ...
     &       ispin,ne,irecl,nmax,nini,b1,b2,b3,iz2)
...
#ifdef MPI_USE
      call MPI_BARRIER(MPI_COMM_WORLD,ierr)
      call MPI_REDUCE(rnnfield,rnnfield_tot,1,MPI_DOUBLE_PRECISION,
     &                MPI_SUM,0,MPI_COMM_WORLD,ierr)
      call MPI_REDUCE(rnnfield_bottom,rnnfield_bottom_tot,1,
     &                MPI_DOUBLE_PRECISION,
     &                MPI_SUM,0,MPI_COMM_WORLD,ierr)
      call MPI_REDUCE(rnfield,rnfield_tot,iz2*nk,
     &                MPI_DOUBLE_PRECISION,
     &                MPI_SUM,0,MPI_COMM_WORLD,ierr)
      call MPI_REDUCE(rflux,rflux_tot,iz2*nk,
     &                MPI_DOUBLE_PRECISION,
     &                MPI_SUM,0,MPI_COMM_WORLD,ierr)
      call MPI_REDUCE(rminsv,rminsv_tot,iz2*nk,
     &                MPI_DOUBLE_PRECISION,
     &                MPI_SUM,0,MPI_COMM_WORLD,ierr)
      call MPI_REDUCE(recivec,recivec_tot,3*iz2*nk,
     &                MPI_DOUBLE_PRECISION,
     &                MPI_SUM,0,MPI_COMM_WORLD,ierr)
      call MPI_REDUCE(recilat,recilat_tot,3*iz2*nk,
     &                MPI_DOUBLE_PRECISION,
     &                MPI_SUM,0,MPI_COMM_WORLD,ierr)
...
#ifdef MPI_USE
        call MPI_BARRIER(MPI_COMM_WORLD,ierr)
        call MPI_REDUCE(chernnumber,chernnumber_tot,1,
     &                  MPI_DOUBLE_PRECISION,
     &                  MPI_SUM,0,MPI_COMM_WORLD,ierr)
        call MPI_REDUCE(berrycurv,berrycurv_tot,nk,
     &                  MPI_DOUBLE_PRECISION,
     &                  MPI_SUM,0,MPI_COMM_WORLD,ierr)
        call MPI_REDUCE(recivec,recivec_tot,3*nk,
     &                  MPI_DOUBLE_PRECISION,
     &                  MPI_SUM,0,MPI_COMM_WORLD,ierr)
        call MPI_REDUCE(recilat,recilat_tot,3*nk,
     &                  MPI_DOUBLE_PRECISION,
     &                  MPI_SUM,0,MPI_COMM_WORLD,ierr)
...
!       elseif(ikubo.eq.1.and. myrank==0) then
     ...
     &                     xberrycurv_kubo,chernnumber,0d0,
     &                     berrymax,berrymin,
     &                     iz2,icd,iz,ivel,ikubo,nprocs)
           endif
         enddo ! loop ie
     ...
     &                    irecl,ecut,nk,nkx,nky,nband,b1,b2,b3,kperiod,
     &                    dSkxky,nini,nmax,xrecivec,xrecilat,kext,
     &                    xberrycurv_kubo,chernnumber_total,0d0,
     &                    berrymax,berrymin,
     &                    iz2,icd,iz,ivel,ikubo,nprocs)
         endif
...
!!$*  write Fukui-Hatsugai n-field Z2 result and diagnostics
     ...
     &            dskxky
      ...
      allocate(partner(nnk),half(nnk),ninteger(nnk))
      partner=0
      half=0
      ninteger=0
      ...
      do i=1,nnk
       ...
       do j=1,nnk
        do m=1,3
         dq(m)=recilat(m,i)+recilat(m,j)
         dq(m)=dq(m)-dnint(dq(m))
        enddo
        if(maxval(dabs(dq)).le.tol)then
         found=found+1
         partner(i)=j
        endif
       enddo
     ...
     &             i,found
      ...
      do i=1,nnk
       ...
       if(partner(partner(i)).ne.i)then
        write(0,*) '*** error - noninvolutive Z2 TR partner',i,
     &             partner(i)
        call vaspberry_fail
       endif
      enddo
      ...
      nsum_bottom=0
      do i=1,nnk
       q(2)=modulo(recilat(2,i),1d0)
       ninteger(i)=nint(rnfield(i))
       if(q(2).lt.0.5d0)then
        half(i)=1
        ntop=ntop+1
        nsum_top=nsum_top+ninteger(i)
       else
        half(i)=-1
        nbottom=nbottom+1
        nsum_bottom=nsum_bottom+ninteger(i)
       endif
      enddo
      ...
      do i=1,nnk
       j=partner(i)
       nres=dabs(rnfield(i)-dble(ninteger(i)))
       npairres=dabs(rnfield(i)+rnfield(j))
       fluxres=dabs(datan2(dsin(rflux(i)+rflux(j)),
     &                    dcos(rflux(i)+rflux(j))))
       maxnres=dmax1(maxnres,nres)
       maxnpairres=dmax1(maxnpairres,npairres)
       maxfluxres=dmax1(maxfluxres,fluxres)
     ...
     &                         maxnres
      write(94,'(A,ES24.16E3)')'# max_nfield_pair_residual=',
     &                         maxnpairres
     ...
     & 'pair_nfield_int_sum,plaquette_checks_pass'
      ...
      do i=1,nnk
       do m=1,3
        q(m)=modulo(recilat(m,i),1d0)
       enddo
       do m=1,3
        kp(m)=q(1)*b1(m)+q(2)*b2(m)+q(3)*b3(m)
       enddo
       j=partner(i)
       nres=dabs(rnfield(i)-dble(ninteger(i)))
  ...
  910 format(I0,6(',',ES24.16E3),2(',',I0),
     ...
     &                     maxnres
      ...
      return
      end subroutine write_z2_field_csv
...
!!$*  subroutine for writing results
     ...
     &                       " (LSORBIT = .FALSE.)"
       ...
       if(nini .eq. nmax) then
        ...
        elseif(iz+ivel+icd .eq. 0.and.ikubo.ne.2)then
     ...
     &   selectivity(n(k)),        (recip)kx        ky        kz"
        ...
        elseif(ikubo .ge. 1)then !standard bare-momentum Kubo
         ...
         if(ikubo.eq.1)then
          ...
          write(32,'(A,ES24.16E3)')'# Chern Number = ',rvari
         else
          write(32,'(A)')'# integration=NONE_K_PATH'
          write(32,'(A)')'# Chern_number=NOT_APPLICABLE_K_PATH'
         endif
         write(32,'(A,I0,A,I0)')'# selected_bands=',nini,':',nmax
         write(32,'(A,I0)')'# intermediate_bands=1:',nband
         write(32,'(A)')'# Omega_n=-2*Im[Dx(n,m)*Dy(m,n)]/gap^2'
         write(32,'(A)')'# summed over m/=n; D=(hbar/m_e)*p'
         write(32,'(A)')'# Cartesian xy component; units angstrom^2'
         write(32,'(A)')"# (cart) kx        ky        kz(A^-1)
...
!!$*  subroutine for get nfield strength and physical flux
     ...
     &                      nbmax,npmax,ecut,ispinor,ispin,ne,
     &                      irecl,nmax,nini,b1,b2,b3,iz2)
      implicit real*8 (a-h,o-z)
      ...
      integer ni,nj,ne,nk,nband,npmax,kperiod,ispin,irecl
      integer ngrid,ilp,itr_sum,nparity

      pi=4d0*datan(1d0)
      ngrid=(2*nbmax(1)+1)*(2*nbmax(2)+1)*
     &      (2*nbmax(3)+1)

      call klpfind(wkk,isp,ik,nk,nband,nkx,nky,wnklist,
     &             ihf,iz,iz2)
      call kindxfind(ikk,isgg,npl,itr,itrim,wnklist,nplist,
     &               wkk,nnk,nband,iz,w_half_klist,
     &               i_half_klist,i_trim_klist,nhf,iz2)
      ...
      do ilp=1,4
       Sijt=(0d0,0d0)
       do ni=nini,nmax
        call get_z2_state(coeff1u,coeff1d,ilp,ni,ikk,npl,
     &       nband,nk,isp,npmax,itrim,itr,wkk,wklist,nbmax,
     &       ispinor,b1,b2,b3,ecut)
        do nj=nini,nmax
         call get_z2_state(coeff2u,coeff2d,ilp+1,nj,ikk,npl,
     &        nband,nk,isp,npmax,itrim,itr,wkk,wklist,nbmax,
     &        ispinor,b1,b2,b3,ecut)
         Sijt(ni-nini+1,nj-nini+1)=
     &        dot_product(coeff1u,coeff2u)+
     &        dot_product(coeff1d,coeff2d)
        enddo
       enddo
...
!!$*  exact reciprocal-space mapping for direct and TR images
      ...
      implicit none
      integer gt(3),gs(3),itheta,ierr,j,ishift
      real*8 wks(3),wkt(3),delta,tol
      ...
      do j=1,3
       ...
       ishift=nint(delta)
       if(dabs(delta-dble(ishift)).gt.tol)ierr=1
       if(itheta.eq.0)then
        gt(j)=gs(j)+ishift
       else
        gt(j)=-gs(j)+ishift
...
!!$*  read and map one spinor state into a common periodic G basis
     ...
     &           ispinor,b1,b2,b3,ecut)
      implicit none
      integer iilp,nni,nband,nk,isp,npmax,ispinor
      integer ikk(5),npl(5),itrim(5),itr(5),nbmax(3)
      integer source_band,itheta,nraw,nsource,ntarget
      integer i,idx,ierr,ios,irec,ngrid
      integer gs(3),gt(3)
      integer, allocatable :: gsource(:,:),gtarget(:,:),seen(:)
      real*8 wkk(3,5),wklist(3,nk),wks(3),wkt(3)
      real*8 b1(3),b2(3),b3(3),ecut
      real*8 norm_in,norm_out,tol
      complex*8, allocatable :: raw(:)
      complex*16 coeffu((2*nbmax(1)+1)*(2*nbmax(2)+1)*
     &                  (2*nbmax(3)+1))
     ...
     &            iilp
      ...
      source_band=nni
      itheta=itr(iilp)
      if(itrim(iilp).ge.1 .and. mod(nni,2).eq.0)then
       source_band=nni-1
       itheta=1
      endif
     ...
     &            nraw
      ...
      ngrid=(2*nbmax(1)+1)*(2*nbmax(2)+1)*
     &      (2*nbmax(3)+1)
      allocate(raw(nraw),gsource(3,npmax),gtarget(3,npmax))
      allocate(seen(ngrid))
      raw=(0.,0.)
      gsource=0
      gtarget=0
      ...
      wks(:)=wklist(:,ikk(iilp))
      wkt(:)=wkk(:,iilp)
      call plindx(gsource,nsource,ispinor,wks,b1,b2,b3,
     ...
     &            norm_in,norm_out
      ...
      deallocate(seen,gtarget,gsource,raw)
      return
      end subroutine get_z2_state
...
!!$*  subroutine for computing optical selectivity in the given k-point
     ...
     &                           ik,wk,selectivity(ik)
      ...
      enddo !ik loop end
#ifdef MPI_USE
      call MPI_ALLREDUCE(selectivity, selectivity_, nk,
     &                   MPI_DOUBLE_PRECISION,
     &                   MPI_SUM, mpi_comm_earth, mpierr)
      selectivity = selectivity_
      call MPI_ALLREDUCE(selectivity_w, selectivity_w_, nk,
     &                   MPI_DOUBLE_PRECISION,
     &                   MPI_SUM, mpi_comm_earth, mpierr)
...
! computing P_x^{nm} and P_y^{nm}
     ...
     &            conjg(coeffn(iplane))*xkgy*coeffm(iplane) ! P_y^{nm}=<u_{n,k}|p_y|u_{m,k}>
       ...
       enddo ! iplane loop end
! Standard A=i<u|grad_k u> curvature; reverse Py is its conjugate.
! Pre-1.3 used unnormalized circular components and was twice this.
       berrycurv_kubo(ik)=berrycurv_kubo(ik)+berrycurv_unit*
     &    kubo_interband_term(cinter_mtrx_x,cinter_mtrx_y,
     &                        ener(nn)-ener(mm))
...
#ifdef MPI_USE
      call MPI_ALLREDUCE(berrycurv_kubo, berrycurv_kubo_, nk,
     &                   MPI_DOUBLE_PRECISION,
     &                   MPI_SUM, mpi_comm_earth, mpierr)
...
! Standard nondegenerate interband contribution. Caller excludes zero gap.
      real*8 function kubo_interband_term(px_nm,py_nm,gap)
      implicit none
      complex*16 px_nm,py_nm
      real*8 gap
     ...
     &             'energy_eV,omega_z_A2,min_gap_eV'
      ...
      do ik=1,nk
       ...
       gap=huge(1d0)
       do m=1,nband
        if(m.ne.nn)gap=min(gap,abs(ener(nn)-ener(m)))
       enddo
       write(96,'(I0,2(",",I0),6(",",ES25.17E3))',iostat=ios)
     &   isp,ik,nn,wklist(:,ik),ener(nn),omega(ik),gap
       if(ios.ne.0)then
        write(0,*) '*** error - cannot write Kubo CSV'
        call vaspberry_fail
       endif
      enddo
      close(96,iostat=ios)
      ...
      if(iz .eq. 0)then
       do ilp=1,5   !ilp
        do ik=1,nk  !ik
         ...
         if (distk .lt. del)then
          ikk(ilp)=ik
          npl(ilp)=nplist(ik)
          else if(abs(1.-distk).lt.del)then
           if (abs(1.-dsqrt(d1**2)).lt.del)then
            ikk(ilp)=ik
            npl(ilp)=nplist(ik)
            isgg(1,ilp)=1
           else if(abs(1.-dsqrt(d2**2)).lt.del)then
            ikk(ilp)=ik
            npl(ilp)=nplist(ik)
            isgg(2,ilp)=1
           endif
          else if(abs(sqrt(2.)-distk).lt.del)then
           ikk(ilp)=ik
           isgg(1,ilp)=1
           isgg(2,ilp)=1
           npl(ilp)=nplist(ik)
      ...
      elseif(iz .eq. 1)then
       do ilp=1,5 !ilp
        do ik=1,nhf !ik
         ...
         elseif (distkt .lt. del .and. distk .gt. del)then
          if(distkm1 .lt. del)then
           ikk(ilp)=i_half_klist(ik) ! -M1
           npl(ilp)=nplist(i_half_klist(ik))
           itrim(ilp)=i_trim_klist(ik)
           isgg(1,ilp)=-1
          elseif(distkm2 .lt. del)then
           ikk(ilp)=i_half_klist(ik) ! -M2
           npl(ilp)=nplist(i_half_klist(ik))
           itrim(ilp)=i_trim_klist(ik)
           isgg(2,ilp)=-1
          elseif(distkm3 .lt. del)then
         ...
         elseif (distktx .lt. del .and. distk .gt. del)then
          if(distkm31 .lt. del)then
         ...
         elseif (distkty .lt. del .and. distk .gt. del)then
          if(distkm32 .lt. del)then
...
! Resolve the task after all options, independent of argument order.
     ...
     &    iwf.ne.0.or.ixt.ne.0.or.it.ne.0)then
        write(0,*) '*** error - -kubo_csv requires Kubo-only mode'
        call vaspberry_fail
       endif
     ...
     &            'omit -ii/-if/-is/-nn'
      ...
      else if(icd.gt.1 .and. TRIM(foname_base) .ne. 'BERRYCURV' )then
       if(icd .eq.1) then
         write(foname,'(A,A)')"CIRC_DICHROISM.",TRIM(foname_base)
       elseif(icd.eq.2) then
         write(foname,'(A,A)')"CIRC_DICHROISM_W.",TRIM(foname_base)
       endif
      else if (icd .gt. 1 .and. TRIM(foname_base) .eq. 'BERRYCURV') then
        if(icd .eq.1) then
        ...
        foname="VEL_EXPT"
      else if (ikubo .ge. 1 .and. TRIM(foname_base) .eq. 'BERRYCURV') then
...
! With Intel ifx/ifort, byte RECL requires -assume byterecl.
     ...
     &      (xrecilat(i,ik),i=1,3)
      ...
      write(6,*)"                  :     BERRYCURV_KUBO.EIG-NN.DAT and"
      write(6,*)"                  :     total Berry curvature (19-22)"
      write(6,*)"                  :     is in BERRYCURV_KUBO.DAT"
      write(6,*)"                  :  Default : -kubo 0 (no calc.)"
      write(6,*)"                  : Standard -2 Im normalization."
      write(6,*)"                  : Bare momentum from pseudo WAVECAR;"
