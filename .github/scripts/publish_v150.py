"""Publish version 1.5.0 only after validation of this exact source commit."""
import json
import os
...
TAG = 'v' + VERSION
PREVIOUS_TAG = 'v1.4.2'
PREVIOUS_COMMIT = '0a3e98680eb4688aad5abd2a2a6ac8ab0bd74a7e'
CHECKS = {
    'ci.yml': frozenset({
    ...
    'intel-mpi-validation.yml': frozenset({'Intel MPI (ifx)', 'Intel MPI (ifort)'}),
    'installation-validation.yml': frozenset({
...
def checked_workflow(name, target, timeout=3600):
...
def release_body(path, target, checks):
    ...
    text += '\nAll four workflows checked this exact release commit. Historical material-convergence studies retain their original provenance.\n'
    return text


def validate_metadata():
    require(Path('VERSION').read_text().strip() == VERSION, 'Incorrect source version')
    require(f'## [{VERSION}] - 2026-09-26' in Path('CHANGELOG.md').read_text(),
    ...
    for path in ('docs/VALIDATION_1.5.0.md', 'docs/releases/v1.5.0.md',
...
def publish(target, checks):
    ...
    payload = {
        ...
        'name': 'VASPBERRY 1.5.0 — simple settings for reproducible postprocessing',
        'body': release_body(Path('docs/releases/v1.5.0.md'), target, checks),
